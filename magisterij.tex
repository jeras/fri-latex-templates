...
Tukaj opisana implementacija algoritma (izvorna koda je v dodatku \ref{koda}) ne išče vseh predslik za vrstico hkrati,
...
Algoritem za izpis predslik (izvorna koda je v dodatku \ref{koda}) je nadaljevanje algoritma za štetje.
...
Posledično lahko linearen algoritem vzame začetek ene od poti in ga
...
Največ algoritmov je namenjenih iskanju stanj GoE,
tako da le-ti zgolj preverjajo obstoj predslik in jih ne štejejo ali izpisujejo.
Naslednja skupina algoritmov sicer izpisuje predslike,
ampak tega ne počne sistematično,
saj je njihov cilje poiskati le eno ali nekaj predslik.
...
Woodsov algoritem deluje po principu sestopanja (glej dodatek \ref{woods}). Polje celic razdeli v vrstice.
...
Nicolay Beluchenko je poiskal večje število stanj GoE velikosti \(11 \times 11\) \cite{FoE}.
Vsaj v enem primeru je uporabljal podoben algoritem kakor Woods \cite{oeis.org/A196447}.
...
Originalni članek je v francoščini, tako da lahko algoritem opišem le na podlagi tega,
kako ga opisujejo drugi \cite{WikiGoE, Bickford2012}.
...
\section{Nesistematično iskanje predslik GoL}
...
Iz navedenega primera je razvidno, da je aplikacija sposobna izpisovati predslike.
Avtorja mi ni uspelo kontaktirati, tako da ne morem podati več podrobnosti.

\section{Sistematični algoritmi za štetje in izpis predslik}

Našel sem le eno aplikacijo, imenovano \emph{RetroGUI}, ki je namenjena štetju in izpisu vseh predslik \cite{Bickford2012}.
Neil Bickford prav tako kakor ostali procesira konfiguracijo
...
Izkažejo se, če je predslik relativno malo (naprimer iskanje GoE).
So pa neprimerni za situacije, ko je predslik relativno veliko,
ker hranijo vse predslike v pomnilniku ves čas izvajanja.
...
Na zadnje, je opisani algoritem zamišljen univerzalno, za poljuben 2D CA,
...
Zanimivi sta predvsem funkciji \verb|ca1d_net| in \verb|ca2d_net|
ki se nahajata proti koncu datoteke: \\
\verb|preimages–2D/ca2d_net.c|
\vspace{10mm}
...
\lstinputlisting[language=C, firstline=30]{preimages-2D/ca2d.h}
\lstinputlisting[language=C, firstline=30]{preimages-2D/ca2d.c}
\lstinputlisting[language=C, firstline=30]{preimages-2D/ca2d_array.h}
\lstinputlisting[language=C, firstline=30]{preimages-2D/ca2d_array.c}
\lstinputlisting[language=C, firstline=30]{preimages-2D/ca2d_base.c}
\lstinputlisting[language=C, firstline=30]{preimages-2D/ca2d_cfg.h}
\lstinputlisting[language=C, firstline=30]{preimages-2D/ca2d_cfg.c}
\lstinputlisting[language=C, firstline=30]{preimages-2D/ca2d_rule.h}
\lstinputlisting[language=C, firstline=30]{preimages-2D/ca2d_rule.c}
\lstinputlisting[language=C, firstline=30]{preimages-2D/ca2d_fwd.h}
\lstinputlisting[language=C, firstline=30]{preimages-2D/ca2d_fwd.c}
\lstinputlisting[language=C, firstline=30]{preimages-2D/ca2d_net.h}
\lstinputlisting[language=C, firstline=30]{preimages-2D/ca2d_net.c}
