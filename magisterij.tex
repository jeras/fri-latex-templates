...
Za klasične 2D CA sta definirani von Neumannova in Mooreova okolica (slika \ref{neighborhood_clasic}).
...
\begin{figure}[htb]
\centerline{\psfig{figure=neighborhood_classic,width=16cm}}
\caption[Klasične 2D okolice.]
{Celica \(c(x,y)\) in pripadajoča okolica \(n(x,y)\) z dimenzijama \(M_x=M_y=3\).}
\label{neighborhood_classic}
\end{figure}

\subsubsection{Conwayev GoL}

To je binarni celični avtomat \(|S|=2\), ki uporablja Mooreovo okolico \(M_x=M_y=3\).
V okolici \(n\) je \(M=M_x \cdot M_y=3 \cdot 3=9\) celic, kar da nabor velikosti \(|S|^9=512\).
V prekrivanju v smereh X in Y je \(M=(M_x-1) \cdot M_y=M_x \cdot (M_y-1)=6\) celic, kar da nabor velikosti \(|S|^9=64\).
V prekrivanjih \(o_{\leftrightarrow}\) in \(o_{\updownarrow}\) v diagonalnih smereh
