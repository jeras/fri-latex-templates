...
velikost končne mreže je definirana s parom \(N={N_x,N_y}\).

Trenutno stanje neke celice \(c_x\) je odvisno od prejšnjega stanja njene okolice \(n_x\).
Podobno se bomo tudi pri obliki okolice omejili na pravokotnik.
Velikost okolice naj bo definirana s parom \(M={M_x,M_y}\)
in število celic v okolici je \(m=M_x \times M_y\). Prostorski odnos med okolico
in celico v prihodnjem stanju avtomata, ki jo ta okolica določa, ni podrobno definiran.
Običajno se smatra, da je celica v sredini okolice, ampak za opisani algoritem to ni
nujno pomembno.

\begin{figure}[htb]
\centerline{\psfig{figure=neighborhood_isometric,width=16cm}}
...
if definicije celičnih avtomatov kakor ločene grafične elemente. In odnosi med
