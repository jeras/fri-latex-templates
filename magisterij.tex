...
\begin{figure}[htb]
\centerline{\includegraphics[width=16cm]{ca110-interaction2}}
...
Christopher Langton je vzel del univerzalnega konstruktorja in ga poenostavil v preprosto zanko
...
Pomembno orodje za analizo časovno in prostorsko diskretnih dinamičnih sistemov je atraktorjevo korito (slika \ref{atractor_basin}).
Andrew Wuensche že leta proučuje atraktorjeva korita naključnih binarnih mrež in celičnih avtomatov \cite{Wuensche1992}.
Atraktorjevo korito je graf, kjer so vozlišča stanja cikličnega končnega CA,
...
Sam atraktor je cikel v grafu korita, vsebuje lahko eno ali več stanj.
Atraktor je časovno periodično stanje v katerem se končna časovna evolucija vsakega končnega diskretnega dinamičnega sistema.
Atraktor predstavlja tudi stabilne objekte v neskončnem CA.
Za GoL obstajajo katalogi takih objektov, kjer so urejeni po velikosti, periodi in hitrosti premikanja.
Kakor primer bi navedel mrtvo ozadje (perioda 1) in drsalca (perioda 4).

\begin{figure}[htb]
\centerline{\psfig{figure=attractor_basin_rule110_conf00010011011111,width=15cm}}
\caption[Atraktorjevo korito.]
{Atraktorjevo korito za elementarno pravilo 100 in konfiguracijo 00010011011111.
Vsebuje osrednji cikel atraktorja ter drevesa, ki rastejo iz cikla in se končujejo z listi (stanja GoE).
Slika je narejena s programom \url{www.ddlab.com}.}
\label{attractor_basin}
\end{figure}

\section{Algoritmi za štetje in izpis predslik 2D CA}
...
\begin{enumerate}[noitemsep,nolistsep]
 ...
 \item DFS algoritmi \cite{WikiDFS} \cite{WikiBacktracking}
...
Bistvo optimalnih algoritmov je, da je proces štetja predslik optimalen.
Za 1D CA je zahtevnost štetja linearno odvisna od velikosti problema \(O(N)\) (\(N\) je število opazovanih celic). 
Proces izpisa predslik ima v povprečju skozi vsa stanja vedno eksponentno rastočo komponento,
saj tudi število vseh stanj sistema raste eksponentno z velikostjo problema.
Optimalni algoritmi tudi omogočajo izpis predslik brez slepih poti,
kar je posledica podrobne analize problema ob štetju.
Zahtevnost izpisa je posledično časovno in pomnilniško linearno odvisna od števila predslik,
tako za 1D kakor za 2D CA.

Raziskave algoritmov za štetje predslik 2D CA sem se lotil s predpostavko,
da je tako kakor za 1D CA tudi tu možno opraviti štetje predslik z linearno zahtevnostjo.
...
od dimenzij polja CA \( O(C^{N_x}) \), in linearno z velikostjo druge dimenzije
(\(C\) je konstanta odvisna od števila stanj celice in velikosti okolice, ne pa tudi od velikosti polja).
Ker pa nisem dokazal optimalnost opisanega algoritma za štetje predslik 2D CA,
...
Večina primerov in sploh ilustracije za GoL bi bili zaradi
\(2^(3 \cdot 3)=512\) možnih stanj okolice preveč kompleksni in nepregledni.
